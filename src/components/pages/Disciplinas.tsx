...
export default function Disciplinas() {
  ...
  const [selectedDisciplina, setSelectedDisciplina] = useState<typeof disciplinas[0] | null>(null)
  ...
  const handleOpenInscripcion = (disciplina: typeof disciplinas[0]) => {
  ...
  const handleInscribir = (socioId: string) => {
                  ...
                  <div style={{ width: '100%', height: '6px', background: 'var(--glass-border)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div
                      style={{
                        height: '100%',
                        width: `${ocupacion}%`,
                        background: estaLleno ? 'var(--error)' : 'var(--accent-primary)',
                  ...
                  {availableSocios.slice(0, 5).map(socio => (
                    <div key={socio.id} className="flex items-center justify-between p-3 rounded-lg border border-glass" style={{ background: 'var(--bg-secondary)', gap: '0.75rem' }}>
                      <div className="flex items-center gap-md" style={{ overflow: 'hidden' }}>
                        <div className="avatar" style={{ background: 'var(--gradient-primary)', flexShrink: 0 }}>{socio.avatar}</div>
