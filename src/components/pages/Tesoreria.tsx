      ...
      <div className="tesoreria-mid-grid mb-8">
        ...
        <div className="glass-panel" style={{ padding: '1.75rem' }}>
          ...
          <div className="p-4 rounded-lg" style={{ background: 'var(--input-bg)', border: '1px solid var(--glass-border)' }}>
              ...
              <div className="flex-col gap-sm">
                {transactions.slice(0, 3).map(tx => (
                  <div key={tx.id} className="flex justify-between items-center py-2 border-b border-glass last:border-0" style={{ flexWrap: 'wrap', gap: '0.25rem' }}>
                    <div>
                      <div className="text-sm font-semibold">{tx.description || tx.concepto}</div>
                      <div className="text-xs text-secondary">{tx.date || tx.fecha}</div>
                    </div>
                    <div className={`font-bold ${tx.type === 'ingreso' || !tx.type ? 'text-success' : 'text-error'}`}>
                      {tx.type === 'ingreso' || !tx.type ? '+' : '-'}{formatCurrency(tx.amount || tx.monto || 0)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <button className="text-sm font-medium mt-4" style={{ color: 'var(--accent-primary)' }}>Ver Historial Completo →</button>
          </div>
        </div>
      </div>
