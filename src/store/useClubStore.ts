...
interface Socio {
  ...
  nombre: string
  dni: string
  categoria: 'Activo' | 'Vitalicio' | 'Cadete' | 'Becado'
...
interface Reserva {
  ...
  fecha: string
  hora: string
  estado: 'Pendiente Seña' | 'Confirmada' | 'Cancelada' | 'Completada'
  señaPagada: number
}
...
interface ClubStore {
  ...
  // Actions
  registerClub: (data: { name: string; location: string; foundation: string }) => void
  toggleTheme: () => void
  payDue: (socioId: string) => void
  addSocio: (socioData: Partial<Socio>) => void
  updateCategoriaSocio: (id: string, nuevaCategoria: Socio['categoria']) => void
  inscribirSocioADisciplina: (socioId: string, disciplinaId: string) => void
  registrarReserva: (reservaData: Partial<Reserva>) => void
  pagarSena: (reservaId: string, monto: number) => void
  registrarIngresoEvento: (eventoData: { nombre: string; descripcion?: string; monto: number }) => void
}
...
const historicalRevenue: RevenueData[] = [
...
const memberGrowth: MemberGrowth[] = [
...
export const useClubStore = create<ClubStore>((set) => ({
  ...
  payDue: (socioId) => set((state) => {
    ...
    if (socio.estado === 'Al día') return state
  ...
  addSocio: (socioData) => set((state) => {
    const newSocio: Socio = {
      ...
      dni: socioData.dni || '',
      categoria: socioData.categoria || 'Activo',
      estado: 'Al día',
      cuota: socioData.cuota || 5000,
  ...
  inscribirSocioADisciplina: (socioId, disciplinaId) => set((state) => {
    const disciplinasActualizadas = state.disciplinas.map(d => {
      if (d.id === disciplinaId && !d.inscriptos.includes(socioId)) {
        if (d.inscriptos.length >= d.cupoMaximo) return d
  ...
  registrarReserva: (reservaData) => set((state) => {
    const newReserva: Reserva = {
      ...
      recursoId: reservaData.recursoId || '',
      reservadoPor: reservaData.reservadoPor || '',
      fecha: reservaData.fecha || '',
      hora: reservaData.hora || '',
      estado: (reservaData.señaPagada || 0) > 0 ? 'Confirmada' : 'Pendiente Seña',
      señaPagada: reservaData.señaPagada || 0
    }
  ...
  pagarSena: (reservaId, monto) => set((state) => {
    ...
    const reservasActualizadas = state.reservas.map(r => {
      if (r.id === reservaId) {
        ...
        return { ...r, señaPagada: r.señaPagada + monto, estado: 'Confirmada' as const }
      }
      return r
    })
